 ...
 *
 * `listCards` fans out: one GET /pokemon?offset=&limit= for the page, then
 * one GET /pokemon/{name} per entry to retrieve sprites + types. The
 * upstream cache interceptor makes the fan-out cheap on repeated visits;
 * the first visit pays N+1 requests (~21 for a page of 20).
 *
 * `getDetails` is a single GET — same endpoint the list fan-out hits, so
 * a card click on a pokémon you just scrolled past is a cache hit.
 */
